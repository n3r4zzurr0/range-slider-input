module.exports = (element, options = {}) => {
  ...
  const currentPosition = (e, node) => {
    return ((node.offsetLeft + (e[`client${options[_orientation] === _vertical ? 'Y' : 'X'}`] - node.getBoundingClientRect()[options[_orientation] === _vertical ? 'top' : 'left']) - (thumbDrag ? ((0.5 - (value[thumbDrag] - options[_min]) / maxWidth) * thumbWidth[thumbDrag]) : 0)) / element.clientWidth) * maxWidth + options[_min]
  }
  ...
  return {
    ...
    step: (s = false) => {
      if (s) {
        ...
        syncThumbWidth()
        setValue('', true)
      } else { return input[0][_step] === _any ? _any : _parseFloat(input[0][_step]) }
    },
    value: (v = false) => {
      if (v) {
        syncThumbWidth()
